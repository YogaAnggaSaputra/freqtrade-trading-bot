#!/usr/bin/env python3
"""Sync feather files to PostgreSQL - fixed password"""
import pandas as pd
...
async def sync():
    ...
    env_password = None
    with open('/home/ngodingyuk/deploy/.env', 'r') as f:
        for line in f:
            if line.startswith('DB_PASSWORD='):
                env_password = line.strip().split('=')[1]
                break
    
    if not env_password:
        env_password = "Changeme123"
    
    print(f"Using DB password from .env: {env_password[:3]}***")
